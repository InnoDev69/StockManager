...
// TOAST
// ─────────────────────────────────────────────────────────────────────────────
function notifySettings(msg, isError = false) {
  NotificationManager.toast(msg, { type: isError ? "error" : "success" });
}
  ...
  if (isPathField(leafName)) {
    ...
    browseBtn.addEventListener("click", async () => {
      if (!window.pywebview?.api?.select_folder) {
        notifySettings("El selector de carpetas no está disponible en este entorno.", true);
        return;
      }
      ...
      try {
        ...
        notifySettings(`No se pudo abrir el explorador: ${err.message}`, true);
      } finally {
  ...
  bindEvents() {
    ...
    this.container.addEventListener("click", async (e) => {
      ...
      try {
        ...
        notifySettings(`Error al guardar "${path[path.length - 1] || topKey}": ${err.message}`, true);
        throw err; 
      } finally {
        btn.disabled = false;
    ...
    this.container.addEventListener("change", async (e) => {
      ...
      if (matrixTable) {
        ...
        try {
          ...
          notifySettings(`Error al guardar "${path.join(" → ")}": ${err.message}`, true);
...
function initProfileForm() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!email) return notifySettings("El correo es requerido.", true);
    ...
    try {
      ...
      notifySettings(err.message, true);
    } finally {
      btn.disabled = false;
    }
  });
...
function initPasswordForm() {
  ...
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    if (!checkMatch()) return notifySettings("Las contraseñas no coinciden.", true);
