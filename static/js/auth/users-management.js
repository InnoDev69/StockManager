...
async function confirmApproveApplication(userId) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      setTimeout(() => {
        modal.remove();
        NotificationManager.toast('Solicitud aprobada correctamente', { type: 'success' });
        loadApplications();
      ...
      NotificationManager.toast(data.error || 'Error al aprobar', { type: 'error' });
    }
  } catch {
    btn.disabled = false;
    btn.style.opacity = '1';
    btn.innerHTML = origText;
    NotificationManager.toast('Error de conexión', { type: 'error' });
  }
}
  ...
  fetch(`/api/applications/${userId}/reject`, { method: 'POST' })
    .then(res => {
      if (res.ok) {
        ...
        setTimeout(() => {
          modal.remove();
          NotificationManager.toast(' Solicitud rechazada correctamente', { type: 'warning' });
          loadApplications();
          loadUsers(currentPage);
        }, 200);
      } else {
        return res.json().then(data => {
          ...
          btn.style.opacity = '1';
          btn.innerHTML = origText;
          NotificationManager.toast(data.error || 'Error al rechazar', { type: 'error' });
        });
      }
    })
    .catch(() => {
...
async function saveUser() {
  ...
  if (!username || !email) { NotificationManager.toast("Completá los campos requeridos", "warning"); return; }
  if (!id && !password)    { NotificationManager.toast("La contraseña es obligatoria para nuevos usuarios", "warning"); return; }
  if (password && password.length < 6) { NotificationManager.toast("La contraseña debe tener al menos 6 caracteres", "warning"); return; }
  if (email === window.CURRENT_USER && status === "0") { NotificationManager.toast("No podés desactivar tu propio usuario", "warning"); return; }
